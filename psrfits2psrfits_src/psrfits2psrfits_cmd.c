...
static Cmdline cmd = {
  /***** -b: The number of bits in each output sample (2, 4, or 8) */
  /* numbitsP = */ 1,
  /* numbits = */ 8,
  /* numbitsC = */ 1,
...
usage(void)
{
  fprintf(stderr,"%s","   [-b numbits] -o outfile [-i] [--] infiles ...\n");
  fprintf(stderr,"%s","      Converts 16-bit PSRFITS search-mode data to 8, 4, or 2-bit PSRFITS\n");
  fprintf(stderr,"%s","         -b: The number of bits in each output sample (2, 4, or 8)\n");
  fprintf(stderr,"%s","             1 int value between 2 and 8\n");
  fprintf(stderr,"%s","             default: `8'\n");
  fprintf(stderr,"%s","         -o: Base name of the output psrfits file\n");
  fprintf(stderr,"%s","             1 char* value\n");
  fprintf(stderr,"%s","         -i: Invert the band\n");
  fprintf(stderr,"%s","    infiles: Input PSRFITS file names (in time order)\n");
  fprintf(stderr,"%s","             1...100 values\n");
  fprintf(stderr,"%s","  version: 08Dec15\n");
  fprintf(stderr,"%s","  ");
  exit(EXIT_FAILURE);
}
/**********************************************************************/
...
parseCmdline(int argc, char **argv)
{
  ...
  for(i=1, cmd.argc=1; i<argc; i++) {
    ...
    if( 0==strcmp("-b", argv[i]) ) {
      int keep = i;
      cmd.numbitsP = 1;
      i = getIntOpt(argc, argv, i, &cmd.numbits, 1);
      cmd.numbitsC = i-keep;
      checkIntLower("-b", &cmd.numbits, cmd.numbitsC, 8);
      checkIntHigher("-b", &cmd.numbits, cmd.numbitsC, 2);
      continue;
    }
